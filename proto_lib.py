...
def show_provenance(key_list,data_list,tw,heat_map_list,indices=None,multicolor=False,seq_length=1,region_index=None,weekday_indices=None,single_day=False,showgoal=False,pos=None,neg=None,comparison=False,summ_indices=None,trailing=False):
    ...
    labels = [x+1 for x in range(0,math.ceil(len(data_list[0])/float(tw)),2)]
    plt.xticks(np.arange(0, len(data), tw*2),labels,fontsize=14)
    ...
    if "Past Daily TW" in summarizer_type:
        ...
        if "10-minute" in proto_type:
            ...
            for i in range(len(summarizer_list)):
                ...
                if ',' in summarizers or len(attr_list) == 1:
                    summarizers = summarizers.split(',')
                    for j in range(len(summarizers)):
                        ...
                        if j != len(summarizers)-1:
                            ...
                            if time_window == 7:
                                ...
                                pre_blurb = " in month "
                            summary += pre_blurb + str(int(first_index)-1)
                else:
            ...
            if type(summarizers) is not list:
                if time_window == 7:
                    pre_blurb = " in week "
                elif time_window == 30:
                    pre_blurb = " in month "
            ...
            if time_window == 7:
                ...
                blurb = pre_blurb + " they did in week " + str(int(second_index)-1)
            elif time_window == 30:
                #blurb = pre_blurb + " they did the month before"
                blurb = pre_blurb + " they did in month " + str(int(second_index)-1)
            summary += blurb + '.'
            #input(summary)
        elif proto_type == "GC":
...
def generateSESTW(attr,key_list,sax_list,letter_map_list,alpha,alpha_sizes,tw,TW,quick=False,start_day=None,end_day=None,age=None,activity_level=None,arm_filepath=None,food_items=False):
...
def generateEC(attr,key_list,sax_list,tw_sax_list,alpha,alpha_sizes,letter_map_list,TW,tw,age=None,activity_level=None,arm_filepath=None,other_index=None):
...
def generateGC(attr,attr_list,key_list,data_list,sax_list,tw_sax_list,alpha,alpha_sizes,letter_map_list,TW,tw,prev_start_day,start_day,end_day,age=None,activity_level=None,day_list=None,arm_filepath=None,other_index=None):
    weather_flag = False
...
def generateCB(attr,attr_list,key_list,full_sax_rep,tw_sax_list,sax_list,data_list,letter_map_list,alpha_sizes,alpha,tw,TW,age,activity_level,arm_filepath=None,quick=False,quick_prov=None,week_index=None,constraint=False,thres=None):
    #tmp = []
    ...
    if "Activity" not in key_list and TW != None and TW != "hours":
        ...
        try:
            ...
            [cluster_data, indices, tw_index, clusters] = series_clustering(full_sax_rep,tw_sax_list,tw,flag=chunked_sax,week_index=tw_index,thres_=thres)
        except TypeError:
            cluster_data = None
            indices = None
        ...
        if cluster_data != None and len(cluster_data) != None:
            ...
            for i in range(len(key_list)):
                ...
                for letter in summary_data:
                    ...
                    try:
                        ...
                        print(letter,letter_map_list,alpha_sizes)
                        summarizer = evaluateSAX(letter,letter_map_list[i],alpha_sizes[i])
            ...
            if quantifier_list != None:
                ...
                if quick:
                    return [cluster_summary,[truth],cluster_data,indices_]
                if quick_prov:
                    import string
                    ...
                    return [cluster_summary,[truth],cluster_data,indices_,summ_indices]
    ...
    if tw > 0.04:
        ...
        if len(summary_list) != 0:
            ...
            for i in range(len(summary_list)):
                ...
                if attr == "MyFitnessPal" and arm_filepath != None:
                    with open(arm_filepath,"a",newline='') as csvfile:
                        pid = pid_list[df_index]
                        datawriter = csv.writer(csvfile)
                        key_list_str = ""
        ...
        for i in range(len(summarizers)):
            ...
            try:
                if age != None and activity_level != None:
                    ...
                    #print(summarizers,letter_map,alpha_size)
                ...
                print("Error in dynamic_nest")
                input([i,attr_list,summarizers,letter_map,alpha_size])
    ...
    for i in range(n):
        ...
        for j in range(len(summarizers)):
            ...
            if flag == "compare":
                ...
                if summarizers[j] in conclusion_map.values():
                    conclusion_ = conclusion_map[conclusion[j]]

                if type(conclusion_) is list:
                    conclusion_ = conclusion_[0]

                #input([conclusion,summarizers[j]])
                #print(conclusion_,summarizers[j])
                if conclusion_ == summarizers[j]:
                    muS = 1
                else:
                    muS = 0
...
def get_data_list(index_list,dataset,demo=False,constraints=False,bank=False):
    '''
    Inputs:
    ...
    if dataset == "Stock Market Data":
        ...
        if demo:
            ...
            data_folder = "calorie_shuffle"
            columns_ = ["date","Calories"]
            column_list = ["date","Calories"]
        else:
    ...
    for i in range(len(index_list)):
        ...
        if dataset == "MyFitnessPal":
            if not demo and "log" not in csv_list[data_index] and not bank:
                continue
...
def create_database(sax_list,num_sax,letter_map_list,tw,alpha_sizes,prefix,weekdays=None):
    ...
    for i in range(0,len(sax_list[0]),tw):
        ...
        if i+tw > len(weekday_list):
            substring = sax_list[0][i:len(weekday_list)]
            # print(i)
            # input(substring)
        ...
        for j in range(0,len(substring)):
            ...
            for k in range(len(sax_list)):
                ...
                # print(substr)
                # print(len(weekday_list),index,len(letter_map_list),k,substr[j],len(alpha_sizes))
...
def get_patterns(sax_list,num_sax,letter_map_list,tw,alpha_sizes,prefix,path,cygwin_path,min_sup,weekdays=None):
    ...
    # Create .data file
    input_file = prefix + ".ascii"
    output_file = prefix + ".data"
    makebin = ["wsl","makebin", input_file, output_file]
    subprocess.check_output(makebin,stderr=subprocess.STDOUT)

    # Create .conf file
    file_path = prefix
    getconf = ["wsl","getconff", "-i", file_path, "-o", file_path]
    subprocess.check_output(getconf,stderr=subprocess.STDOUT)
    # Create .tpose and .idx files
    #close(file_path + ".idx")
    exttpose = ["wsl","exttpose", "-i", file_path, "-o", file_path, "-l", "-s", "0", "-x"]
    error = False
    ...
    if not error:
        ...
        spade = ["wsl","spade.exe", "-e", "1", "-i", path+file_path, "-s", minimum_support, "-o", "-w", TW, "-u", "1","-Z",seq_length]
...
def series_clustering(sax_rep,tw_sax_list,window_size,alpha_size=5,flag=None,week_index=None,thres_=None):
    '''
    Inputs:
    ...
    thres = float(sum(avg_array))/len(avg_array) + 1 if thres_ == None else thres_
    #input([thres_,thres])
    #if thres_ != None:
        #thres = thres_
